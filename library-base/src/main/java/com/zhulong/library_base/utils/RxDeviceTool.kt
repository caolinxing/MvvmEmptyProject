...
object RxDeviceTool {
    ...
    val uniqueSerialNumber: String
        get() {
            ...
            LogUtil.d("详细序列号", "$manuFacturer-$phoneName-$serialNumber")
    ...
    fun getDeviceIdIMEI(context: Context): String? {
        ...
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
            ToastUtil.showToast(context,"请先获取读取手机设备权限")
            return null
        }
    ...
    fun getSimSerialNumber(context: Context): String? {
        val tm = context.getSystemService(Context.TELEPHONY_SERVICE) as TelephonyManager
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
            ToastUtil.showToast(context,"请先获取读取手机设备信息权限")
            return null
        }
...
// samsung 品牌
    ...
    fun ThroughArray(res: HashMap<*, *>) {
        ...
        while (ite.hasNext()) {
            ...
            LogUtil.d("MSG_AUTH_COMPLETE", "$key： $value")
        }
    }
    ...
    val macAddress: String
        get() {
            ...
            LogUtil.d("mac", "interfaceName=" + networkInterface.name + ", mac=" + macAddress)
